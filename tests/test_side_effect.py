import pytest
import pandas as pd
from src.side_effect.side_effect import preprocess_text, get_drugs, get_comment_dict, remove_comment
...
def test_process_text():
    ...
    check_punc = [any(char in string.punctuation for char in text) for text in processed_text]
    assert not all(check_punc), "Cleaned text contains text with punctuation!"
    check_number = [any(char in string.digits for char in text) for text in processed_text]
...
def test_get_drugs():
    drugs = ['adderall', 'clonidine', 'concerta', 'dexedrine', 'intuniv', 'qelbree', 'ritalin', 'strattera', 'vyvanse', 'wellbutrin', 'dexstrostat', 'kapvay']
    test_data = load_test_data()
...
def test_get_commit_dict():
    ...
    assert all(check_dict), f"Something wrong with adding column or transforing dataframe to dictionary!"
